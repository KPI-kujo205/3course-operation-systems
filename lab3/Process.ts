...
export class Process {
	...
	constructor(
		...
		const pageTableSize = Math.floor(
			Math.random() * (maxTableSize - minTableSize + 1) + minTableSize,
		);
		this.pageTable = Array.from(
			{ length: pageTableSize },
			() => new PageTableEntry(),
		);
		...
		this.requestLimit = Math.floor(
			Math.random() * (maxProcessRequests - minProcessRequests + 1) +
				minProcessRequests,
		);
	...
	accessMemoryPages(
		minMemoryRequests: number,
		maxMemoryRequests: number,
		mmu: MMU,
	) {
		const memoryRequests = Math.floor(
			Math.random() * (maxMemoryRequests - minMemoryRequests + 1) +
				minMemoryRequests,
		);
		...
		for (let i = 0; i < memoryRequests; i++) {
			...
			if (!config.DISABLE_LOGGING) {
				console.log(
					`Process ${this.requestCount}/${this.requestLimit}, page index ${pageIndex}, read/write ${isWrite}`,
				);
			}
		}
	}
